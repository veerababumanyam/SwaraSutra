...
export const setSafeLocalStorage = (key: string, value: any) => {
  try {
    ...
    console.warn(`LayaVani Storage: Write Error:`, error);
  }
};
...
export const getActiveApiKey = (): string | null => {
  try {
    const saved = localStorage.getItem("LayaVani_api_key");
...
export const saveApiKey = (key: string) => {
  try {
    localStorage.setItem("LayaVani_api_key", key);
  } catch (e) {
    console.warn("LayaVani: Failed to save API key", e);
  }
};
...
export class RateLimiter {
  ...
  async acquire(): Promise<void> {
    ...
    if (this.timestamps.length >= this.maxRPM) {
      ...
      console.warn(`LayaVani: RPM limit reached. Waiting ${Math.round(waitTime / 1000)}s...`);
...
export async function retryWithBackoff<T>(
  ...
  try {
    ...
    console.warn(`LayaVani: [${model || 'unknown'}] Transient error (${msg.slice(0, 60)}...). Retrying in ${actualDelay}ms (${maxRetries} attempts left)`);
...
export const generatePDF = async (elementId: string, title: string) => {
  ...
  try {
    ...
    pdf.text("Generated by LayaVani AI Studio", pdfWidth / 2, pdfHeight - 10, { align: "center" });

    pdf.save(`${title.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'LayaVani_lyrics'}.pdf`);
  } catch (error) {
